...
[Assistencial](docs/ASSISTENCIAL.md).
...
- A exportação usa páginas de 5.000 registros por padrão (`SGU_EXPORT_PAGE_SIZE`).
...
- O arquivo é gerado integralmente em temporário antes da resposta. Falhas viram JSON; nenhum BOM isolado ou relatório parcial é entregue como sucesso.
...
Detalhes atuais de integridade, ordenação, formatos, paginação e validação estão em
[Exportação de relatórios](docs/RELATORIOS_EXPORTACAO.md).

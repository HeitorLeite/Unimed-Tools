...
A ordenação usa todos os desempates da projeção, com campo técnico curto no SGU quando necessário. A ordem ajustada na prévia também é aplicada na exportação simples. Erros recebidos como Blob são decodificados antes de encerrar a operação. Ver [Exportação de relatórios](RELATORIOS_EXPORTACAO.md).

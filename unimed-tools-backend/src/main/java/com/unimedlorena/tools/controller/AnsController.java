/*
 * Responsabilidade: Expõe os contratos multipart do Corretor de Rede ANS.
 */
...
public class AnsController {
  ...
  public AnsController(AnsService svc) {
    this.svc = svc;
  }
  ...
  public ResponseEntity<byte[]> filtrarCsvs(
    ...
    @RequestParam(
      value = "municipio",
      required = false
    ) MultipartFile municipio,
    @RequestParam(
      value = "prestador",
      required = false
    ) MultipartFile prestador,
  ...
  private ResponseEntity<byte[]> buildResponse(
    AnsService.Resultado r,
    String nomeOriginal
  ) throws Exception {
    String stats = mapper.writeValueAsString(r.stats());
    String nome =
      nomeOriginal != null
        ? nomeOriginal.replace(".txt", "_filtrado.txt")
        : "rede_filtrada.txt";

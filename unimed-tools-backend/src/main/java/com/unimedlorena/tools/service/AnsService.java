/*
 * Responsabilidade: Implementa a leitura das fontes de filtro e a remoção de registros do TXT posicional ANS.
 */
...
public class AnsService {
  ...
  /*
   * O arquivo de rede é um layout posicional legado. Estes índices são
   * contratos de negócio da ANS, não limites arbitrários de implementação.
   */
  private static final int POS_CNPJ_INI = 0;
  private static final int POS_CNPJ_FIM = 14;
  private static final int POS_CNES1_INI = 94;
  private static final int POS_CNES1_FIM = 101;
  private static final int POS_CNES2_INI = 109;
  private static final int POS_CNES2_FIM = 116;
  private static final Charset TXT_CHARSET = StandardCharsets.ISO_8859_1;

  // ── Regex para abas separadas (formato antigo) ─────────────────────────────
  private static final Pattern P_ECNES_ABA = Pattern.compile(
    "CNPJ/CPF:\\s*(\\d+)",
    Pattern.CASE_INSENSITIVE
  );
  private static final Pattern P_CNES_ABA = Pattern.compile(
    "CNES.*?:\\s*(\\d+)\\s*-\\s*(?:CNPJ[^:]*:\\s*)?(\\d+)",
    Pattern.CASE_INSENSITIVE
  );
  private static final Pattern P_MUN_ABA = Pattern.compile(
    "CNES:\\s*(\\d+).*?CNPJ/RAZ[^:]*:\\s*(\\d+)",
    Pattern.CASE_INSENSITIVE
  );
  private static final Pattern P_PREST_ABA = Pattern.compile(
    "CNPJ/RAZ[^:]*:\\s*(\\d+).*?CNES:\\s*(\\d*)",
    Pattern.CASE_INSENSITIVE
  );
  private static final Pattern P_AVISO_ABA = Pattern.compile(
    "CNES:\\s*(\\d+)",
    Pattern.CASE_INSENSITIVE
  );
  private static final Pattern P_MISTO_ABA = Pattern.compile(
    "CNPJ[/\\w]*[^:]*:\\s*(\\d{9,14})",
    Pattern.CASE_INSENSITIVE
  );
  ...
  public Resultado processarXlsx(MultipartFile xlsx, MultipartFile txt)
    throws Exception {
    Filtros f = new Filtros();

    // O try-with-resources garante a liberação do ZIP interno do XLSX.
    try (Workbook wb = new XSSFWorkbook(xlsx.getInputStream())) {
      for (int i = 0; i < wb.getNumberOfSheets(); i++) {
        ...
        if (tipo != null) {
          ...
          StringBuilder conteudoAba = new StringBuilder();
          for (Row row : ws) {
            for (Cell cell : row) {
              String valor = cellStr(cell);
              if (!valor.isBlank()) conteudoAba.append(valor).append('\n');
            }
          }
          carregarTipoAba(f, tipo, conteudoAba.toString());
        } else if (
          nomeAba.equalsIgnoreCase("Erros") ||
          nomeAba.toLowerCase().startsWith("erros")
        ) {
          // O formato mais novo reúne as mensagens da ANS em texto livre.
  ...
  private void carregarTextoLivre(Filtros f, Sheet ws) {
    ...
    for (int r = 1; r <= ws.getLastRowNum(); r++) {
      ...
      if (m.find()) {
        f.ecnes.add(zfill(m.group(1), 14));
        continue;
      }
    ...
    f.addContagem(
      "Prestador",
      f.prestadorExato.size() + f.prestadorCuringa.size() - antesPrest
    );
  ...
  public Resultado processarCsvs(
    MultipartFile csvEcnes,
    MultipartFile csvCnes,
    MultipartFile csvMun,
    MultipartFile csvPrest,
    MultipartFile csvAviso,
    MultipartFile txt
  ) throws Exception {
    Filtros f = new Filtros();
    ...
    switch (tipo) {
      ...
      case "CNES" -> {
        ...
        for (String l : texto.split("\n")) {
          ...
          if (m.find()) f.cnes.add(
            zfill(m.group(2), 14) + "|" + zfill(m.group(1), 7)
          );
        }
      ...
      case "Prestador" -> {
        ...
        f.addContagem(
          "Prestador",
          f.prestadorExato.size() + f.prestadorCuringa.size() - antes
        );
      }
      case "Aviso" -> {
  ...
  private Resultado filtrar(MultipartFile txtFile, Filtros f) throws Exception {
    // A codificação faz parte do contrato do arquivo posicional da ANS.
    String[] linhas = new String(txtFile.getBytes(), TXT_CHARSET).split(
      "\n",
      -1
    );
    ...
    BufferedWriter w = new BufferedWriter(
      new OutputStreamWriter(out, TXT_CHARSET)
    );
  ...
  private boolean deveRemover(String linha, Filtros f) {
    ...
    String cnpj =
      tam >= POS_CNPJ_FIM
        ? zfill(c.substring(POS_CNPJ_INI, POS_CNPJ_FIM).strip(), 14)
        : null;
  ...
  private String classificarAba(String nome) {
    ...
    return switch (n) {
      ...
      case "aviso" -> "Aviso";
      default -> n.startsWith("erros_") || n.startsWith("misto")
        ? "ErrosMisto"
        : null;
    };
  }
  ...
  private String ler(MultipartFile f) throws Exception {
    return new String(f.getBytes(), TXT_CHARSET);
  }

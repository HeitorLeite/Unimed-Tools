/*
 * Responsabilidade: Implementa no backend a análise de prefixos e despesas zeradas em XML TISS.
 */
...
public class XmlService {
  ...
  public XmlAnaliseResponse analisar(
    String content,
    boolean corretor,
    boolean removedor
  ) {
    List<PrefixoErro> prefixos = corretor
      ? analisarPrefixos(content)
      : List.of();
  ...
  private List<PrefixoErro> analisarPrefixos(String content) {
    ...
    while (m.find()) {
      erros.add(
        new PrefixoErro(m.start(), m.group(2) + m.group(3), m.group(3))
      );
    }
      ...
      if (vm.find() && Double.parseDouble(vm.group(1)) == 0.0) {
        blocos.add(
          new BlocoZerado(
            ...
            tag(bloco, "ans:codigoTabela") +
              " / " +
              tag(bloco, "ans:codigoProcedimento"),
  ...
  public String corrigir(String content, boolean corretor, boolean removedor) {
    if (corretor) {
      content = P_PREFIXO.matcher(content).replaceAll(
    ...
    if (removedor) {
      content = P_DESPESA.matcher(content).replaceAll(m -> {
        ...
        return vm.find() && Double.parseDouble(vm.group(1)) == 0.0
          ? ""
          : m.group();
      });
    }
    ...
    boolean possuiOutrasDespesasVazia = P_OUTRAS_DESPESAS_VAZIA.matcher(
      content
    ).find();

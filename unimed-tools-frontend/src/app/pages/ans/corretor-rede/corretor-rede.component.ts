/**
 * Orquestra o envio das fontes de filtro e do TXT posicional ao módulo ANS.
 */
...
interface LogEntry {
  msg: string;
  tipo: 'info' | 'ok' | 'error' | 'warn' | 'section';
}
interface Stats {
  lidas: number;
  removidas: number;
  mantidas: number;
  chavesPorTipo: Record<string, number>;
}
interface CsvSlot {
  key: string;
  label: string;
...
export class CorretorRedeComponent {
  ...
  csvSlots: CsvSlot[] = [
    ...
    {
      key: 'prestador',
      label: 'Prestador (Err 8521)',
      hint: 'Filtra por CNPJ + CNES (curinga aceito)',
      file: null,
    },
  ...
  readonly formatSize = formatarTamanhoArquivo;

  // ── Upload XLSX ──────────────────────────────────────────────────────────────
  onDragOverX(e: DragEvent) {
    e.preventDefault();
    this.dragOverXlsx = true;
  }
  onDragLeaveX() {
    this.dragOverXlsx = false;
  }
  onDropX(e: DragEvent) {
    e.preventDefault();
    this.dragOverXlsx = false;
  ...
  removerCsv(slot: CsvSlot) {
    slot.file = null;
    this.resetResultado();
  }

  // ── Upload TXT ───────────────────────────────────────────────────────────────
  onDragOverT(e: DragEvent) {
    e.preventDefault();
    this.dragOverTxt = true;
  }
  onDragLeaveT() {
  ...
  processar() {
    ...
    if (this.modo === 'xlsx') {
      ...
      for (const slot of this.csvSlots)
        if (slot.file) {
          form.append(slot.key, slot.file);
          this.addLog(`${slot.label}: ${slot.file.name}`, 'info');
        }
    }
    ...
    this.api.upload(endpoint, form).subscribe({
      next: (event: any) => {
        ...
        if (event.type === HttpEventType.Response) {
          try {
            this.stats.set(JSON.parse(event.headers.get('X-Stats')));
          } catch {}
      ...
      error: async (err) => {
        ...
        try {
          const text =
            err.error instanceof Blob ? await err.error.text() : JSON.stringify(err.error);
  ...
  baixar() {
    ...
    baixarBlob(this.downloadBlob, this.downloadNome);
  }

  get chavesPorTipo() {
    return Object.entries(this.stats()?.chavesPorTipo ?? {});
  }
}

...
export class CustomReportComponent implements OnInit {
    ...
    this.reports
      ...
      .subscribe({
        next: (response) => {
          ...
          const preferred = (this.tool?.colunasPreview ?? []).filter((column) =>
            this.records.some((row) => Object.prototype.hasOwnProperty.call(row, column)),
          );
  ...
  download(): void {
    ...
    this.reports
      .exportar(this.tool.apiNome, formato, this.parameters(), filename)
      .pipe(
        finalize(() => {
      ...
      .subscribe({
        next: (event) => {
          if (event.type === HttpEventType.Response && event.body) {
            baixarBlob(event.body, `${filename}.${formato}`);
          }
        },
        error: (error: any) =>

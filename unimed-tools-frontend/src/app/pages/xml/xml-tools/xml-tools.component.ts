/**
 * Coordena seleção, análise e download dos XMLs processados no navegador.
 */
...
export class XmlToolsComponent {
  ...
  get nomesGerados(): string[] {
    ...
    if (this.tipoNomeDownload === 'corrigido') {
      return this.resultadosDownload.map(
        (resultado) => `${this.removerExtensaoXml(resultado.nome)}_corrigido.xml`,
      );
    }
  ...
  get textoBotaoDownload(): string {
    ...
    return this.resultadosDownload.length > 1 ? 'Baixar tudo em ZIP' : 'Baixar arquivo';
  }
  ...
  get totalOutrasDespesasVazias() {
    return this.resultados.reduce((s, r) => s + r.outrasDespesasVazias.length, 0);
  }

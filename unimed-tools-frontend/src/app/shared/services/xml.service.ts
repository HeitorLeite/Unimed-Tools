import { ArquivoResultado, ArquivoZip, CorretorXmlError, RemovedorBlock, OutrasDespesasVazia } from '../models/xml.models';
...
/**
 * Implementa no navegador as regras de análise, correção e empacotamento dos XMLs.
 */
import { Injectable } from '@angular/core';
import { corrigirLotesDuplicados } from '../utils/xml-lotes.utils';
...
export class XmlService {
  ...
  async processarLote(
    ...
    const lotes = corrigirLotesDuplicados(conteudos);
    ...
    for (let i = 0; i < files.length; i++) {
      let content = lotes[i].correctedContent;
      ...
      if (fazerRemovedor) {
        ...
        content = content.replace(/<ans:despesa>[\s\S]*?<\/ans:despesa>/g, (bloco) => {
          const vm = VP.exec(bloco);
      ...
      resultados.push({
        ...
        guiasRenomeadas,
        lotesRenumerados: lotes[i].lotesRenumerados,
  ...
  analisarRemovedor(content: string): { blocks: RemovedorBlock[]; correctedContent: string } {
    ...
    let correctedContent = content.replace(/<ans:despesa>[\s\S]*?<\/ans:despesa>/g, (bloco) => {
      const vm = valorPattern.exec(bloco);
      return vm && parseFloat(vm[1]) === 0 ? '' : bloco;
    });

    correctedContent = this.removerOutrasDespesasVazias(correctedContent).replace(/\n\s*\n/g, '\n');
